...
function isFhevmRelayerSDKType(
  ...
  const hasZamaEthereumConfig = objHasProperty(o, "ZamaEthereumConfig", "object", trace);
  const hasSepoliaConfig = objHasProperty(o, "SepoliaConfig", "object", trace);
  if (!hasZamaEthereumConfig && !hasSepoliaConfig) {
    trace?.("RelayerSDKLoader: relayerSDK missing network config (ZamaEthereumConfig or SepoliaConfig).");
    return false;
  }
